...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_n', type=str, default=None, help='')
    parser.add_argument('--dataset_name', type=str, default='mobile', help='')
    ...
    if args.experiment == 'e2e-tgt' or  args.experiment == 'e2e-tgt-pos' or args.experiment == 'e2e-tgt-tree' or \
            ...
            Model_FILE = args.exp_n +"_"+  args.dataset_name
            if args.augment == 1:
                Model_FILE = Model_FILE + "_augment"
            logging_dir = os.path.join(folder_name, 'runs', Model_FILE)
            Model_FILE = os.path.join(folder_name, Model_FILE)
            app = f" --dataset_name={args.dataset_name} --augment={args.augment} " \
                  f"--dataset_config_name {args.dataset_config_name} --task {args.task}"
            app += " " + args.app


    COMMANDLINE = f"/data2/songyiwen/workspace/user_profile/human_traj_diffusion/improved-diffusion/scripts/run_clm.py \
            ...
            --gradient_accumulation_steps {args.gradient_accumulation_steps} --exp_n {args.exp_n} " \
