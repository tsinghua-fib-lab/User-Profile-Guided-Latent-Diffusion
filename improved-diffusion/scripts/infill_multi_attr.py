...
from infill_util import langevin_multi
...
def main():
    ...
    # model
    base_dir = '/data2/songyiwen/'
    model_dir1 = '/data2/songyiwen/human_traj_diffusion/classifier_models/gender_mobile/checkpoint-7000'
    model_dir2 = '/data2/songyiwen/human_traj_diffusion/classifier_models/home_mobile/checkpoint-20000'
    model_control1 = Classifier_GPT2.from_pretrained(model_dir1).cuda()
    model_control2 = Classifier_GPT2.from_pretrained(model_dir2).cuda()
    
    # 控制label文件
    control_label_lst = [[32392,13732]]

    control_constraints = []
    for label_class in control_label_lst:
        label1 = [-100] * 169 + [int(x) for x in label_class] #带上start token
        label_ids1 = th.tensor(label).unsqueeze(0) # torch.Size([1, 170])
        label2 = [-100] * 169 + [int(x) for x in label_class]
        label_ids2 = th.tensor(label).unsqueeze(0)

        debug_lst = [args.k, args.coef]
        langevin_fn_selected = partial(langevin_multi, debug_lst, model_control1, model_control1, model3.cuda(),
