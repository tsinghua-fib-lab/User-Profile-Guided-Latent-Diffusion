...
class Classifier_GPT2(GPT2PreTrainedModel):
    ...
    def __init__(self, config, diffusion=None):
        ...
        self.week_embedding = nn.Embedding(8, config.n_embd)
        self.hour_embedding = nn.Embedding(25, config.n_embd)
    ...
    def forward(
        ...
        if input_weeks is not None and input_hours is not None:
            input_weeks_emb = self.week_embedding(input_weeks)
            input_hours_emb = self.hour_embedding(input_hours)
            input_embs = input_embs + input_weeks_emb + input_hours_emb
        else:
            pos = torch.arange(input_embs.shape[1]).to(self.device).expand((1, -1))
            hour_ids = torch.clone(pos)
            hour_ids[0][1:] = (hour_ids[0][1:]-1) % 24+1
            hour_ids[0][-1] = 0
            week_ids = torch.clone(pos)
            for i in range(7):
            ...
            input_embs = input_embs + week_emb + hour_emb
    ...
    def eval_acc(
        ...
        t = torch.from_numpy(np.array([t for i in range(input_embs.shape[0])])).to(input_embs.device)
        t_mask = (t >= 0)
        input_embs_rand = self.diffusion.q_sample(input_embs, t)
        input_embs[t_mask] = input_embs_rand[t_mask]
        t[~t_mask] = self.train_diff_steps
        ...
        lm_logits = self.lm_head2(hidden_states)
        pred, idx = lm_logits[:,-1,:].max(1)

        return {'pred':np.array(idx.cpu()),
                'data':np.array(input_ids[:,-1].cpu())}
