...
class GaussianDiffusion:
    ...
    def training_losses_e2e(self, model, t, model_kwargs=None, noise=None):
        ...
        x_start_mean = model.model.module.get_embeds(input_ids) # x0
        ...
        x_start = self.get_x_start(x_start_mean, std) # x1
        ...
        terms["mse"] = mean_flat((target - model_output) ** 2) # Lsimple
        terms["Lsimple_loss"] = terms["mse"]

        model_out_x_start = self.x0_helper(model_output, x_t, t)['pred_xstart'] # pred_x1

        # for debugging
        if debug:
            lst = []
            logits = get_logits(model_out_x_start)
            cands = th.topk(logits, k=1, dim=-1)
            sample = cands.indices
            for seq in sample:
                tokens = seq.squeeze(-1)
                lst.append(tokens.cpu().numpy())

        t0_mask = (t == 0)
        t0_loss = mean_flat((x_start_mean - model_out_x_start) ** 2) # loss between emb
        terms["mse"] = th.where(t0_mask, t0_loss, terms["mse"])

        out_mean, _, _ = self.q_mean_variance(x_start, th.LongTensor([self.num_timesteps - 1]).to(x_start.device))
        tT_loss =  mean_flat(out_mean ** 2) #  part of Lsimple when t=T.
        decoder_nll = self.token_discrete_loss(x_start, get_logits, input_ids) # cross entropy loss between the logits and input tokens
        
        terms["loss"] = terms["mse"] + (decoder_nll + tT_loss)
        terms["Lsimple_loss"] += tT_loss
        terms["emb_loss"] = t0_loss
        terms["word_emb_loss"] = decoder_nll
    ...
    def calc_bpd_loop_emb(self, model, x_start, clfip_denoised=True, model_kwargs=None,
                          denoised_fn=None):
        """
