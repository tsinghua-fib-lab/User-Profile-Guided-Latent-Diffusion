...
class TrainLoop:
    ...
    def run_loop(self):
        ...
        data = cycle(self.data)
        eval_data = cycle(self.eval_data)
        ...
        while (
            ...
            data_batch_ = next(data)
            data_batch_ = data_batch_["data"].to(dist_util.dev())
            cond = {}
            cond['input_ids'] = data_batch_[:,:,0,...]
            cond['input_hour'] = data_batch_[:,:,1,...]
            cond['input_week'] = data_batch_[:,:,2,...]

            # batch: torch.Size([64, 64, 16])
            # cond['input_ids']: torch.Size([64, 64])
            ...
            if self.eval_data is not None and self.step % self.eval_interval == 0:
                data_batch_eval = next(eval_data)
                data_batch_eval = data_batch_eval["data"].to(dist_util.dev())
                cond_eval = {}
                cond_eval['input_ids'] = data_batch_[:,:,0,...]
                cond_eval['input_hour'] = data_batch_[:,:,1,...]
                cond_eval['input_week'] = data_batch_[:,:,2,...]
    ...
    def forward_only(self, cond):
        with th.no_grad():
            ...
            for i in range(0, cond['input_ids'].shape[0], self.microbatch):
                ...
                bs = micro_cond['input_ids'].shape[0]
    ...
    def forward_backward(self, cond):
        ...
        for i in range(0, cond['input_ids'].shape[0], self.microbatch):
            ...
            Lsimple_loss = (losses["Lsimple_loss"] * weights).mean()
            emb_loss = (losses["emb_loss"] * weights).mean()
            word_emb_loss = (losses["word_emb_loss"] * weights).mean()

            self.writer.add_scalar(f'Loss', loss, self.step)
            self.writer.add_scalar(f'Lsimple_loss', Lsimple_loss, self.step)
            self.writer.add_scalar(f'emb_loss', emb_loss, self.step)
            self.writer.add_scalar(f'word_emb_loss', word_emb_loss, self.step)
